...
public
class EntityAISpawner extends EntityAIBase implements IEliteAI
{
	...
	private
	EntityAISpawner( EntityLiving entity, NBTTagCompound spawnerTag )
	{
		...
		if( spawnerTag != null ) {
			...
			this.mobSpawnerLogic.setEntityId( new ResourceLocation( entityId ) );
		}
	...
	void initialize( EntityLiving entity )
	{
		...
		entity.getEntityAttribute( SharedMonsterAttributes.MOVEMENT_SPEED ).applyModifier( new AttributeModifier(
			UUID.randomUUID( ), "Spawner speed boost",
			Config.get( ).ELITE_AI.SPAWNER_SPEED_BOOST, 1
		) );
		entity.getEntityAttribute( SharedMonsterAttributes.MAX_HEALTH ).applyModifier( new AttributeModifier(
			UUID.randomUUID( ), "Spawner health boost",
			Config.get( ).ELITE_AI.SPAWNER_HEALTH_BOOST, 0
		) );
	}
	...
	static {
		...
		spawnLogic.setEntityId( new ResourceLocation( "Pig" ) );
	...
	private static
	class MobSpawnerAILogic extends MobSpawnerBaseLogic
	{
		...
		public
		MobSpawnerAILogic( EntityLiving entity )
		{
			this.theEntity = entity;
		}
		...
		void broadcastEvent( int id )
		{
			if( id == 1 ) { // Timer reset event
				for( Entity entity : this.theEntity.world.getEntitiesWithinAABBExcludingEntity( this.theEntity, this.theEntity.getEntityBoundingBox( ).expand( this.aiSpawnRange + 4.0, 2.0, this.aiSpawnRange + 4.0 ) ) ) {
