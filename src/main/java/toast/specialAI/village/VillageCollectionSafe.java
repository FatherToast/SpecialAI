...
public class VillageCollectionSafe extends VillageCollection
{
    ...
    private final List<VillageSafe> loadedVillageList = Lists.<VillageSafe>newArrayList();
	...
	public void tick() {
        ...
        if (this.firstTick || this.tickCounter % 40 == 0) {
	        ...
	        for (ListIterator<Village> itr = this.getVillageList().listIterator(); itr.hasNext();) {
	        	village = itr.next();
	        	if (!VillageSafe.class.equals(village.getClass())) {
	        		village = this.convertVillage(village);
	        		itr.set(village);
	        	}
        ...
        if (Properties.get().VILLAGES.HOUSE_REP && this.tickCounter % 50 == 0) {
        ...
        if (Properties.get().VILLAGES.REFRESH_HOUSES && this.tickCounter % 810 == 0) {
        	for (Village village : this.loadedVillageList) {
	...
	public void addToVillagerPositionList(BlockPos pos) {
    	super.addToVillagerPositionList(pos);
    }

    @Override
	public void readFromNBT(NBTTagCompound tag) {
        NBTTagList villageList = tag.getTagList("Villages", 10);
        tag.removeTag("Villages"); // No need to load villages again
