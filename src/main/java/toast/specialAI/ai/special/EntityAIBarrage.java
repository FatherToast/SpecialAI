...
public class EntityAIBarrage extends EntityAIBase implements ISpecialAI {
    ...
    public void initialize(EntityLiving entity) {
        ...
        entity.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).applyModifier(new AttributeModifier(UUID.randomUUID(), "Barrager health boost", 20.0, 0));
    }
    ...
    public void startExecuting() {
        ...
        this.theEntity.worldObj.playSound(null, new BlockPos(this.theEntity), SoundEvents.BLOCK_STONE_BUTTON_CLICK_ON, this.theEntity.getSoundCategory(), 1.0F, 1.0F / (this.theEntity.getRNG().nextFloat() * 0.4F + 0.8F));
    }
    ...
    public void updateTask() {
        ...
        if (this.state == EntityAIBarrage.STATE_START) {
            ...
            if (this.attackTime <= 0) {
            	...
            	this.attackDir = this.attackDir.addVector(0.0, Math.sqrt(this.attackDir.xCoord * this.attackDir.xCoord + this.attackDir.zCoord * this.attackDir.zCoord) * 0.2, 0.0);
                this.attackTime = 60;
                this.state = EntityAIBarrage.STATE_SHOOT;
            }
        }
        else if (this.state == EntityAIBarrage.STATE_SHOOT) {
            this.theEntity.getLookHelper().setLookPosition(this.theEntity.posX + this.attackDir.xCoord, this.theEntity.posY + this.theEntity.getEyeHeight(), this.theEntity.posZ + this.attackDir.zCoord, 100.0F, 100.0F);
            if (this.attackTime % 5 == 0) {
                EntityArrow arrow = new EntityTippedArrow(this.theEntity.worldObj, this.theEntity.posX, this.theEntity.posY + this.theEntity.getEyeHeight(), this.theEntity.posZ);
                arrow.shootingEntity = this.theEntity;
                arrow.setDamage(this.arrowDamage + this.theEntity.getRNG().nextGaussian() * 0.5 + this.theEntity.worldObj.getDifficulty().getDifficultyId() * 0.22);
                arrow.setThrowableHeading(this.attackDir.xCoord, this.attackDir.yCoord, this.attackDir.zCoord, 1.8F, 20.0F);
                if (this.theEntity.isBurning()) {
                    arrow.setFire(100);
                }

                this.theEntity.worldObj.playSound(null, new BlockPos(this.theEntity), SoundEvents.ENTITY_ARROW_SHOOT, this.theEntity.getSoundCategory(), 1.0F, 1.0F / (this.theEntity.getRNG().nextFloat() * 0.4F + 0.8F));
